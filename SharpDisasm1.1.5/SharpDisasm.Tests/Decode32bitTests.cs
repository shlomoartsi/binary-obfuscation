...
//0000000000000014 83e2df           and edx, 0xffffffdf     
        ...
        public void Test_AND_Decode()
        {
            for (byte i = 0; i <= 0xf; i++)
            {
                ...
                var disasm = new Disassembler(new byte[] {
                0x81 ,(byte)(0xe0 + i) ,0x03, 0x04, 0xf1, 0xf2,   //and ecx, 0xf2f10403
                }, ArchitectureMode.x86_32);



                var insn = disasm.NextInstruction();
                var instructionString = insn.ToString();
                Debug.WriteLine(instructionString);

            }
        }

        [TestMethod]
        public void Test_PartialInstructionReturnsError()
        {
            var disasm = new Disassembler(new byte[] {
                0x81 ,0xe1 ,0x03, 0x04, 0xf1,   //error operation missing last operand of - and ecx, 0xffff0000
            }, ArchitectureMode.x86_32);
            ...
            Assert.IsTrue(insn.Error, "partial instruction bytes should generate error");
        }

        [TestMethod]
        public void Test_DecodeJumpRegister()
        {
            for (int i = 0; i < 0x08; i++)
            {
                var disasm = new Disassembler(new byte[] {0xff,(byte)(0xe0+i)}, ArchitectureMode.x86_32);
                var insn = disasm.NextInstruction();

                Assert.IsTrue(insn != null &&  insn.Mnemonic == ud_mnemonic_code.UD_Ijmp &&
                    insn.Operands.Length > 0 && insn.Operands[0].Base == ud_type.UD_R_EAX + i);
                Debug.WriteLine("Test_DecodeJumpRegister: " + insn.ToString());
            }

        }

        [TestMethod]
        public void Test_jmp_DecodeInstruction()
        {
            var disasm = new Disassembler(new byte[] { 0xE9, 0x04, 0x03, 0x02, 0x01 } ,ArchitectureMode.x86_32);
            var insn = disasm.NextInstruction();

            Assert.IsTrue((ulong)insn.Operands[0].Value == 0x01020304);
        }

        [TestMethod]
        public void Test_ja_DecodeInstruction()
        {
            var disasm = new Disassembler(new byte[] { 0x0F, 0x87, 0x04, 0x03, 0x02, 0x01 }, ArchitectureMode.x86_32);
            var insn = disasm.NextInstruction();
